...
class AccuracyLogger:
    ...
    def log(self, y_hat: torch.Tensor, y: torch.Tensor) -> None:
        pred = int(y_hat.item())
        target = int(y.item())
        self.data[target]["count"] += 1
        self.data[target]["correct"] += int(pred == target)
...
def collate_patch_gcn(batch: list[dict]) -> dict:
    if len(batch) != 1:
        raise ValueError("PatchGCN training currently expects batch_size=1")
    return batch[0]
...
def build_loader(dataset, training: bool, weighted: bool) -> DataLoader:
    ...
    return DataLoader(dataset, batch_size=1, sampler=sampler, collate_fn=collate_patch_gcn, num_workers=0)


def move_sample_to_device(sample: dict) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    features = sample["features"].to(device)
    edge_index = sample["edge_index"].to(device)
    edge_weight = sample["edge_weight"].to(device)
    label = sample["label"].view(1).to(device)
    return features, edge_index, edge_weight, label
...
def summarize_split(name: str, loader: DataLoader, model: nn.Module, n_classes: int, loss_fn: nn.Module):
    ...
    slide_ids = loader.dataset.slide_data["slide_id"].astype(str).tolist()
    ...
    with torch.inference_mode():
        for batch_idx, sample in enumerate(loader):
            ...
            probs[batch_idx] = y_prob.cpu().numpy()[0]
...
def run_epoch(
    ...
    for batch_idx, sample in enumerate(loader):
        features, edge_index, edge_weight, label = move_sample_to_device(sample)
        logits, _, y_hat, _, _ = model(features, edge_index, edge_weight)
        loss = loss_fn(logits, label)
        total_loss += float(loss.item())
        total_error += float(y_hat.item() != label.item())
        acc_logger.log(y_hat, label)
        ...
        if (batch_idx + 1) % 20 == 0:
            print(
                f"batch {batch_idx}, loss: {loss.item():.4f}, label: {label.item()}, "
                f"num_nodes: {features.size(0)}, num_edges: {edge_index.size(1)}"
            )
...
def run_patch_gcn_training(
    ...
    for epoch in range(2 if config.smoke else config.max_epochs):
        ...
        run_epoch(epoch, train_loader, model, optimizer, loss_fn, n_classes=2)
        val_summary = summarize_split("val", val_loader, model, 2, loss_fn)
    ...
    if checkpoint_path.exists():
        model.load_state_dict(load_torch_checkpoint(checkpoint_path))
    else:
    ...
    test_summary = summarize_split("test", test_loader, model, 2, loss_fn)
    ...
    metrics.update(
        {
            ...
            **build_dynamic_count_metrics(config.label_counts, config.split_sizes, config.task_spec),
            ...
            "mean_epoch_time_sec": float(np.mean([row["duration_sec"] for row in epoch_rows])) if epoch_rows else 0.0,
            "num_epochs_ran": len(epoch_rows),
