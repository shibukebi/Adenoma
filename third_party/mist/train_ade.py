...
def main():
    ...
    parser.add_argument('--save_dir', default=None, type=str, help='Directory to save trained weights')
    ...
    bags_csv_train = args.dataset_train if os.path.isfile(args.dataset_train) else os.path.join(
        'datasets', args.dataset_train, args.dataset_train + '.csv')
    bags_csv_val = args.dataset_val if os.path.isfile(args.dataset_val) else os.path.join(
        'datasets', args.dataset_val, args.dataset_val + '.csv')
    ...
    best_score = 0
    save_path = args.save_dir or os.path.join('weights', datetime.date.today().strftime("%m%d%Y"))
    os.makedirs(save_path, exist_ok=True)
    run = len(glob.glob(os.path.join(save_path, '*.pth')))
    pd.Series(vars(args)).to_json(os.path.join(save_path, 'train_args.json'), indent=2)
    history = []
    for epoch in range(1, args.num_epochs):
        ...
        history.append({
            ...
            'val_loss': test_loss_bag,
            'accuracy': accuracy,
            'f1_macro': F1score["macro"],
            'f1_micro': F1score["micro"],
            'average_score': avg_score,
            'auc_mean': float(np.mean(aucs)),
            **{'auc_class_{}'.format(i): auc_value for i, auc_value in enumerate(aucs)},
        })
