...
def get_bag_feats(csv_file_df, args):
    ...
    if feats_csv_path in FEATURE_CACHE:
        feats = FEATURE_CACHE[feats_csv_path]
    else:
        df = pd.read_csv(feats_csv_path)
        feats = df.to_numpy(dtype=np.float32)
        FEATURE_CACHE[feats_csv_path] = feats
    feats = shuffle(feats)
...
def test(epoch, test_df, milnet, args):
    ...
    result_df.to_csv(os.path.join(result_path, 'test_pred.csv'), index=False)

    acc_value = accuracy_score(label_array, prediction_array)
    print(acc_value)

    classes = args.class_names.split('|') if args.class_names else [str(i) for i in range(args.num_classes)]
    ...
    cm_save_name = os.path.join(result_path, '{}_cm.png'.format(epoch))
    ...
    auc_value,fprs, tprs, roc_auc, _, thresholds_optimal = multi_label_roc(
        epoch, test_labels, test_predictions, args.num_classes, args.output_dir)
    fpr_macro = fprs["macro"]
    tpr_macro = tprs["macro"]
    roc_auc_macro = roc_auc["macro"]
    np.save(os.path.join(args.output_dir, 'fpr_macro.npy'), fpr_macro)
    np.save(os.path.join(args.output_dir, 'tpr_macro.npy'), tpr_macro)
    np.save(os.path.join(args.output_dir, 'roc_auc_macro.npy'), roc_auc_macro)


    pr_curves = get_precision_recall(test_labels, test_predictions)
    display_PRcurve(epoch, pr_curves, args.num_classes, args.output_dir)
    ...
    np.save(os.path.join(args.output_dir, 'recall_macro.npy'), recall_macro)
    np.save(os.path.join(args.output_dir, 'precision_macro.npy'), precision_macro)
    np.save(os.path.join(args.output_dir, 'pr_auc_macro.npy'), pr_auc_macro)
...
def multi_label_roc(epoch, labels, predictions, num_classes, output_dir, pos_label=1):
    # fprs = []
    # tprs = []
    ...
    plt.savefig(os.path.join(output_dir, '{}_ROC.png'.format(epoch)))
    # plt.show()
    # --------------------------------------------------------------------------------------------------------------#
...
def display_PRcurve(epoch, pr_curves, n_classes, output_dir):
    if plt is None:
        return
    lw = 2
    plt.figure(figsize=(12, 8), dpi=777)
    ...
    plt.legend(loc="lower right")
    plt.savefig(os.path.join(output_dir, '{}_PRcurve.png'.format(epoch)))
    # plt.show()
...
def main():
    ...
    parser.add_argument('--output_dir', default='inference', type=str, help='Directory to save inference outputs')
    parser.add_argument('--class_names', default='', type=str, help='Class names joined by |')
    ...
    bags_csv = args.dataset if os.path.isfile(args.dataset) else os.path.join('datasets', args.dataset, args.dataset + '.csv')
