...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prepare dual-stream DS-MIL manifests for paired magnification features."
    )
    ...
    parser.add_argument("--stream-a-name", required=True)
    parser.add_argument("--stream-b-name", required=True)
...
def collect_feature_ids(feature_dir: Path) -> tuple[set[str], set[str]]:
    h5_ids = {path.stem for path in (feature_dir / "h5_files").glob("*.h5")}
    pt_ids = {path.stem for path in (feature_dir / "pt_files").glob("*.pt")}
    return h5_ids, pt_ids
...
def main() -> None:
    ...
    feat_a_h5_ids, feat_a_pt_ids = collect_feature_ids(feature_dir_a)
    feat_b_h5_ids, feat_b_pt_ids = collect_feature_ids(feature_dir_b)
    ...
    for row in label_rows:
        ...
        if record is None:
            continue
        record["stream_a_name"] = args.stream_a_name
        record["stream_b_name"] = args.stream_b_name
        dataset_rows.append(record)

        slide_id = str(record["slide_id"])
        if (
            slide_id in feat_a_h5_ids
            and slide_id in feat_a_pt_ids
            and slide_id in feat_b_h5_ids
            and slide_id in feat_b_pt_ids
        ):
    ...
    split_stats = {
        ...
        "stream_a_name": args.stream_a_name,
        "stream_b_name": args.stream_b_name,
        "missing_stream_a_feature_h5": len([row for row in dataset_rows if row["slide_id"] not in feat_a_h5_ids]),
        "missing_stream_a_feature_pt": len([row for row in dataset_rows if row["slide_id"] not in feat_a_pt_ids]),
        "missing_stream_b_feature_h5": len([row for row in dataset_rows if row["slide_id"] not in feat_b_h5_ids]),
        "missing_stream_b_feature_pt": len([row for row in dataset_rows if row["slide_id"] not in feat_b_pt_ids]),
