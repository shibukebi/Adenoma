...
CONFIG_PATH="${CONFIG_PATH:-${PROJECT_ROOT}/config/patch_gcn_ssl_2p5x.env}"
...
if [[ -z "${results_dir}" ]]; then
    results_dir="${PATCH_GCN_RESULTS_ROOT}/fold-${fold}"
fi
...
"${CLAM_PYTHON}" "${PROJECT_ROOT}/scripts/prepare_patch_gcn_ssl_data.py" \
  ...
  --dataset-csv "${PATCH_GCN_DATASET_CSV}" \
  --ready-csv "${PATCH_GCN_READY_CSV}" \
  --split-dir "${PATCH_GCN_SPLIT_DIR}" \
  --fold "${fold}" \
...
cmd=(
  "${CLAM_PYTHON}"
  "${PROJECT_ROOT}/scripts/train_patch_gcn_ssl.py"
  --ready-csv "${PATCH_GCN_READY_CSV}"
  --split-dir "${PATCH_GCN_SPLIT_DIR}"
  --feature-dir "${PATCH_GCN_FEATURE_DIR}"
  --patch-dir "${PATCH_GCN_PATCH_DIR}"
