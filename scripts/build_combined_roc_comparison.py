...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build a combined ROC comparison plot for multiple MIL/CLAM runs.")
...
def run_specs() -> list[dict[str, str]]:
    return [
        {
            "label": "MIL 2.5X",
            "predictions_csv": "/data15/data15_5/yuexin2/adenoma/outputs/clam_ssl_vs_others_2p5x/fold-0/predictions.csv",
            "color": "#1f77b4",
            "linestyle": "-",
        },
        {
            "label": "MIL 20X",
            "predictions_csv": "/data15/data15_5/yuexin2/adenoma/outputs/clam_ssl_vs_others_20x/fold-0/predictions.csv",
            "color": "#ff7f0e",
            "linestyle": "-",
        },
        {
...
def main() -> None:
    ...
    plt.title("Fold-0 ROC Comparison: MIL vs CLAM-SB @ 2.5X / 20X")
    plt.grid(alpha=0.25)
    plt.legend(loc="lower right")
    plt.tight_layout()

    png_path = output_dir / "roc_comparison_mil_vs_clamsb_2p5x_20x_fold0.png"
    pdf_path = output_dir / "roc_comparison_mil_vs_clamsb_2p5x_20x_fold0.pdf"
    csv_path = output_dir / "roc_comparison_mil_vs_clamsb_2p5x_20x_fold0.csv"
    json_path = output_dir / "roc_comparison_mil_vs_clamsb_2p5x_20x_fold0.json"
    ...
    write_json(
        json_path,
        {
            ...
            "runs": summary_rows,
            "note": "SSL is treated as the positive class for all four ROC curves.",
        },
    )
