...
physical_extent_patch_size="${PHYSICAL_EXTENT_PATCH_SIZE:-${PATCH_SIZE:-256}}"

mode="full"
data_h5_dir="${FULL_PATCH_H5_DIR}"
...
while (($#)); do
    case "$1" in
        --smoke)
            mode="smoke"
            data_h5_dir="${SMOKE_PATCH_H5_DIR}"
            csv_path="${SMOKE_FEATURE_INPUT_CSV}"
            feat_dir="${SMOKE_FEATURE_DIR}"
            shift
            ;;
        --csv-path)
            csv_path="$2"
...
timing_dir="${STAGE_TIMING_DIR:-${OUTPUT_ROOT}/stage_timing_${target_magnification}x_uni}"
timing_path="${timing_dir}/${feature_run_name}.json"
mkdir -p "${timing_dir}"
