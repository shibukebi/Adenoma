...
available_csv="${PROJECT_ROOT}/data/adenoma_yx_feature_input_10x_available.csv"
...
while true; do
  ...
  if [[ "${row_count}" -gt 0 ]]; then
    "${PROJECT_ROOT}/scripts/run_extract_features_10x.sh" --csv-path "${available_csv}"
  else
    echo "No available 10X patch h5 files yet."
  fi
  ...
  if [[ "${patch_count}" -gt 0 && "${pt_count}" -ge "${patch_count}" ]]; then
    echo "10X features caught up with current patches."
  fi
