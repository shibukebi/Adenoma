...
CONFIG_PATH="${CONFIG_PATH:-${PROJECT_ROOT}/config/route_lowmag_1x.env}"
...
task_name="${TASK_NAME:-ssl_vs_others_1x_uni}"
k_sample="${K_SAMPLE:-8}"
...
"${CLAM_PYTHON}" "${PROJECT_ROOT}/scripts/prepare_clam_ssl_1x_data.py" \
  --label-csv "${LABEL_CSV}" \
...
cmd=(
    "${CLAM_PYTHON}"
    "${PROJECT_ROOT}/scripts/train_clam_ssl_1x.py"
    --dataset-csv "${CLAM_DATASET_CSV}"
    --ready-csv "${CLAM_READY_CSV}"
    --split-dir "${CLAM_SPLIT_DIR}"
    --feature-dir "${LOWMAG_FEATURE_DIR}"
