...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Select false-negative and false-positive slides for heatmaps.")
...
def ranked_subset(df: pd.DataFrame, top_k: int) -> pd.DataFrame:
    if df.empty:
        return df.copy()
    ranked = df.sort_values("prob_ssl", ascending=False).head(top_k).copy()
...
def main() -> None:
    ...
    summary = {
        ...
        "available_counts": {key: int(len(value)) for key, value in categories.items()},
