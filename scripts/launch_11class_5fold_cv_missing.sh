...
LAUNCH_DIR="${RESULT_ROOT}/launcher_logs/$(date +%Y%m%d_%H%M%S)"
mkdir -p "${LAUNCH_DIR}"
...
run_job() {
  ...
  shift 2
  mkdir -p "${result_dir}"
  if [[ -f "${result_dir}/metrics.json" || -f "${result_dir}/1.pth" ]]; then
...
for fold in ${FOLDS}; do
  for mag in 2p5x 5x 10x 20x; do
    ...
    enqueue "TransMIL_${mag}_fold${fold}" "${RESULT_ROOT}/TransMIL/11class/${mag}/fold-${fold}" \
      "${CLAM_PYTHON}" "${PROJECT_ROOT}/scripts/train_transmil_ssl.py" \
        --ready-csv "${READY_CSV}" \
        --split-dir "${SPLIT_DIR}" \
        --feature-dir "${FEATURE_ROOT}/${mag}" \
        --results-dir "${RESULT_ROOT}/TransMIL/11class/${mag}/fold-${fold}" \
        --fold "${fold}" \
        --max-epochs "${MAX_EPOCHS}" \
        --seed "${SEED}" \
        --lr "${LR}" \
        ...
        --early-stopping \
        --task-mode adenoma_11class \
        --task-name "adenoma_11class_${mag}_transmil_fold${fold}"

    enqueue "DSMIL_${mag}_fold${fold}" "${RESULT_ROOT}/DSMIL/11class/${mag}/fold-${fold}" \
      "${CLAM_PYTHON}" "${PROJECT_ROOT}/scripts/train_dsmil_single_11class.py" \
        --ready-csv "${READY_CSV}" \
        --split-dir "${SPLIT_DIR}" \
        --feature-dir "${FEATURE_ROOT}/${mag}" \
        --results-dir "${RESULT_ROOT}/DSMIL/11class/${mag}/fold-${fold}" \
        --fold "${fold}" \
        --max-epochs "${MAX_EPOCHS}" \
        --seed "${SEED}" \
        --lr "${LR}" \
        ...
        --early-stopping \
        --task-mode adenoma_11class \
        --task-name "adenoma_11class_${mag}_dsmil_fold${fold}"
  done
