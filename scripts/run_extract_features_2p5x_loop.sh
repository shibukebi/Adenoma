...
available_csv="${PROJECT_ROOT}/data/adenoma_yx_feature_input_2p5x_available.csv"
...
while true; do
  ...
  row_count="$("${CLAM_PYTHON}" - <<'PY'
import csv
path='/data15/data15_5/yuexin2/adenoma/data/adenoma_yx_feature_input_2p5x_available.csv'
...
PY
  ...
  if [[ "${row_count}" -gt 0 ]]; then
    "${PROJECT_ROOT}/scripts/run_extract_features_2p5x.sh" --csv-path "${available_csv}"
  else
    echo "No available 2.5X patch h5 files yet."
  fi
  ...
  if [[ "${patch_count}" -gt 0 && "${pt_count}" -ge "${patch_count}" ]]; then
    echo "2.5X features caught up with current patches."
  fi
