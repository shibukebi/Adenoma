...
from utils.utils import calculate_error  # noqa: E402
...
from dsmil.dataset import collate_dual_stream
...
def build_loader(dataset, training: bool, weighted: bool) -> DataLoader:
    ...
    return DataLoader(dataset, batch_size=1, sampler=sampler, collate_fn=collate_dual_stream, **kwargs)
...
def compute_dsmil_loss(results: dict, label: torch.Tensor, loss_fn: nn.Module) -> tuple[torch.Tensor, dict[str, float]]:
    bag_logits = results["bag_logits"]
    max_instance_logits = results["max_instance_logits"]
    bag_loss = loss_fn(bag_logits, label)
    max_loss = loss_fn(max_instance_logits, label)
...
def run_epoch(epoch: int, loader: DataLoader, model: nn.Module, optimizer: torch.optim.Optimizer, loss_fn: nn.Module, n_classes: int):
    ...
    for batch_idx, (data_a, data_b, label) in enumerate(loader):
        data_a = data_a.to(device)
        data_b = data_b.to(device)
        label = label.to(device)

        _, _, y_hat, _, results = model(data_a, data_b)
        loss, loss_parts = compute_dsmil_loss(results, label, loss_fn)
        ...
        if (batch_idx + 1) % 20 == 0:
            print(
                f"batch {batch_idx}, loss: {loss.item():.4f}, bag_loss: {loss_parts['bag_loss']:.4f}, "
                f"max_loss: {loss_parts['max_loss']:.4f}, label: {label.item()}, "
                f"bag_sizes: ({data_a.size(0)}, {data_b.size(0)})"
            )
...
def summarize_split(name: str, loader: DataLoader, model: nn.Module, n_classes: int, loss_fn: nn.Module):
    ...
    with torch.inference_mode():
        for batch_idx, (data_a, data_b, label) in enumerate(loader):
            ...
            _, y_prob, y_hat, _, results = model(data_a, data_b)
            loss, _ = compute_dsmil_loss(results, label, loss_fn)
            total_loss += float(loss.item())
            total_error += calculate_error(y_hat, label)
